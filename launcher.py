    ...
    while True:
        ...
        if event.type == pygame.USEREVENT:
            ...
            if screensaverActive == True and screensaverCounter >= SCREENSAVER_AFTER_DELAY:
                if (screensaverGamesShown >= ABOUT_IMAGE_FREQUENCY):
                    show_about()
                    screensaverGamesShown = 1
                gameIndex = random.randrange(0, gameCount)
                scroll_right(True)
                screensaverCounter = 0
                screensaverGamesShown = screensaverGamesShown + 1
...
def show_about():
    global screen, ABOUT_IMAGE_INDEX
    if len(ABOUT_IMAGES) > 0:
        for x in range (int(screen.get_width() * 1.5), int(screen.get_width() * .5), -1 * SCROLLRATE):
            ABOUT_IMAGES[ABOUT_IMAGE_INDEX].set_centerx(x)
            render = pygame.sprite.RenderPlain(ABOUT_IMAGES[ABOUT_IMAGE_INDEX])
            render.update()
            render.draw(screen)
            pygame.display.flip()
        screen.fill(BLACK)
        ABOUT_IMAGES[ABOUT_IMAGE_INDEX].reset_centerx(screen)
        render = pygame.sprite.RenderPlain(ABOUT_IMAGES[ABOUT_IMAGE_INDEX])
        render.update()
        render.draw(screen)
        pygame.display.flip()
        ABOUT_IMAGE_INDEX = ABOUT_IMAGE_INDEX + 1
        if ABOUT_IMAGE_INDEX >= len(ABOUT_IMAGES):
            ABOUT_IMAGE_INDEX = 0
        # Then wait for a button to be pressed to continue
        # or return after ABOUT_IMAGE_DELAY seconds, using screen saver event
        finished = False
        timeoutCounter=0
        while (finished == False):
            ...
            if (my_event.type == pygame.KEYDOWN):
                ...
                timeoutCounter+=1
                if timeoutCounter > ABOUT_IMAGE_DELAY:
                    finished = True
